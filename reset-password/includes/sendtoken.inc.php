...
if (isset($_POST['resentsend'])) {
    ...
    */
    ...
    $url = ( (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] == 'on') ? 'https://' : 'http://' ) . $_SERVER['HTTP_HOST'] . rtrim(dirname($_SERVER['PHP_SELF']), "includes") . "?selector=" . $selector . "&validator=" . bin2hex($token);
    ...
    if (!mysqli_stmt_prepare($stmt, $sql)){
        ...
        // Email address not found, but give generic message
        // And add delay as otherwise it's possible to tell that
        // the address does not exist by the almost immediate speed
        // of the error message (vs. when an email is sent to an
        ...
        if (mysqli_stmt_num_rows($stmt) == 0){
            ...
            $_SESSION['STATUS']['resentsend'] = "&#9989;&nbsp;If this email address exists in our records then a password reset notification has been sent to the address specified.";
            header("Location: ../");
